...
class BookingManagerService extends ControllerBase {
  ...
  public function generateSchdules($day) {
    ...
    // $entities =
    // $this->entityTypeManager->getStorage('booking_system_schedule')->loadMultiple();
    // /**
    // *
    ...
    // /**
    // *
    // * @var \Drupal\booking_system\Plugin\Field\FieldType\ScheduleItem
    // $schedule
    // */
  ...
  public function periodIsValid($day, $start_hour, $end_hour, $gap = 15, $intervalle = 15) {
    ...
    foreach ($entities as $entity) {
      ...
      foreach ($schedules as $schedule) {
        ...
        if ($current_schedule['start'] >= $start && $current_schedule['start'] < $end || $current_schedule['end'] >= $start && $current_schedule['end'] <= $end) {
          $isValid = false;
          break;
        }
      }
   ...
   * @throws \Exception set the reservations
   */
  ...
  function getSeats($day, $hour) {
    ...
    $entities = $this->entityTypeManager->getStorage('booking_reservation')->loadMultiple();
    $today = strtotime('today');
    /**
     *
