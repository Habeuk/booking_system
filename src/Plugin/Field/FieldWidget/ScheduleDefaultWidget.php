...
class ScheduleDefaultWidget extends WidgetBase {
  ...
  function massageFormValues($values, $form, $form_state) {
    foreach ($values as &$value) {
      if (!empty($value)) {
        $value["start_hour"] = !empty($value["start_hour"]) ? date("h:i", $value["start_hour"]->getTimestamp()) : '';
        $value["end_hour"] = !empty($value["start_hour"]) ? date("h:i", $value["end_hour"]->getTimestamp()) : '';
        $value["start_date"] = !empty($value["start_date"]) ? $value["start_date"]->getTimestamp() : 0;
        $value["end_date"] = !empty($value["end_date"]) ? $value["end_date"]->getTimestamp() : 0;
